...
/**
 * 银行业务类 开户: 向Account表添加一条记录(新增) 存取款: 修改 Account 的余额(修改), 记录流水表(新增) 转账 : A
 * 账号减少(取款), B账号增加(存款) 查询 : 根据开好查余额
 */
@Service
...
public class BankBiz {
	...
	public void register(int id, String pwd, double money) {
		/**
		 * jdbcTemplate 每个 update 都有 独立的事务控制 try{ 业务代码 ==> 回调 提交操作 } catch (Exception e)
		 * { 回滚操作 } finally { 关闭连接 }
		 * 
		 * 
		 */
